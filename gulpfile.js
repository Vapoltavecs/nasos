...
//======
const svgcolor = () => {
  return src("src/images/src/svgcolor/*.svg")
    .pipe(
      svgmin({
        js2svg: {
          pretty: true,
    ...
    .pipe(
      svgSprite({
        mode: {
          symbol: {
            sprite: "../spritecolor.svg",
          },
        },
      })
    )
    .pipe(dest("src/images/dest/svgcolor/"))
    .pipe(sync.stream());
};
//======
const exportImg = () => {
...
const startWatch = () => {
  ...
  watch(["src/fonts/dest/*.woff"], fonts);
...
exports.dev = series(
  remove,
  parallel(html, style, scripts, fonts, exportImg, server, startWatch)
);
exports.build = series(
  remove,
  parallel(html, style, scripts, fonts, exportImg)
);
